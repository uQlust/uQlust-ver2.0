...
char ConvertState(char *stringState)
{
	if(!strcmp(stringState,"+/+ cis "))
		return '+';
	else
	if(!strcmp(stringState,"-/- cis "))
		return ':';
	else
	if(!strcmp(stringState,"S/S cis "))
		return 'A';
	else
	if(!strcmp(stringState,"S/s cis "))
		return 'A';		
	...
	if(!strcmp(stringState,"S/H cis "))
		return 'B';		
	else
	if (!strcmp(stringState, "H/S cis "))
		return 'B';
	else
	if(!strcmp(stringState,"S/W cis "))
		return 'C';				
	else
	if(!strcmp(stringState,"W/S cis "))
		return 'C';		
	else
	if(!strcmp(stringState,"W/W cis "))
		return 'E';				
	else
	if(!strcmp(stringState,"W/H cis "))
		return 'F';				
	else
	if (!strcmp(stringState, "H/W cis "))
		return 'F';
	else
 	if(!strcmp(stringState," stacked "))
		return 'G';				
	else
	if(!strcmp(stringState,"S/S tran"))
		return 'H';
	else
	if(!strcmp(stringState,"S/H tran"))
		return 'I';		
	else
	if(!strcmp(stringState,"H/S tran"))
		return 'I';		
	else
	if(!strcmp(stringState,"H/H tran"))
		return 'K';		
	else
	if(!strcmp(stringState,"H/W tran"))
		return 'L';		
	else
	if (!strcmp(stringState, "W/H tran"))
		return 'L';
	else
	if (!strcmp(stringState, "W/W tran"))
		return 'M';
	else
		return 'O';
}
...
/* bifurcated  bond  (also belongs to 12 families, but with more restraits)*/
					...
					profile[i-1]=ConvertState(type);
					profile[j-1]=ConvertState(type);    
/*
 * 
 * 
...
*/ 
                ...
                if(stack_key>0) { /*rid of base-base stacked case */
                    if(rtn_val[3]<40){
						...
						profile[i-1]=ConvertState(" stacked ");
						profile[j-1]=ConvertState(" stacked ");    
...
//                            printf("increase memory for single H bond (base-sugar)\n");
    ...
    *num_pair_tot = num_bp;
    ...
    bp_network(num_residue, RY, seidx, AtomName, ResName, ChainID, ResSeq,
               Miscs, xyz, bseq, pair_info, Nxyz, orien, org, BPRS,
...
char * write_best_pairs(int num_residue,long num_helixs, long **helix_idx, 
...
/* print out the best base-pairing information */
{
    ...
    char * secStruct;
    secStruct=malloc(sizeof(char)*(num_residue+1));
    
    for(i=0;i<num_residue;i++)
		secStruct[i]='S';
	secStruct[num_residue]='\0';
...
//            fprintf(fout1, "%5ld%5ld%5ld %d%4ld| %s x %s x %s", num_bsp, i_order,
//                    j_order,marker_n, i - helix_idx[ii][1] + 1,  b1, wc, b2);
            ...
            if(wc[0]!='*')
            {
				if(i_order>j_order)
				{
					secStruct[i_order - 1] = 'H';
					secStruct[j_order-1]='H';
					//secStruct[i_order-1]=')';
					//secStruct[j_order-1]='(';
				}
				else
