...
//############################
        ...
        public double ProgressUpdate()
        {
            ...
            return res*0.75+0.25*(double)currentV / maxV;
        }
...
/*                int counter = 0;
        ...
        private void CalcHammingDist(object o)
        {
            ...
            {
                int dist = 0;
                for (int n = 0; n < allStructures[i].Length; n++)
                {
                    dist += allStructures[allStructures.Count - 1][n] ^ allStructures[i][n];
        ...
        private Dictionary<string, List<int>> ClustDist(Dictionary<string, List<int>> dic, List<KeyValuePair<int, int>> sorted, int thresholdH, int vT)
        {
            ...
            s.Load();
            resetEvents = new ManualResetEvent[s.numberOfCores];
            ...
            for (float threshold = (float)thresholdH / vT; threshold <= thresholdH;)
            {
                ...
                for (int i = 0, k = 0; i < hashKeys.Count; i++)
                {
                    ...
                    allStructures.Clear();
                    index.Clear();
                    while (k < sorted.Count && Math.Abs(sorted[i].Key - sorted[k].Key) <= threshold / 4 + 1)
                    {
                        if (avoid[sorted[k].Value])
                        {
                            k++;
                    ...
                    for (int n = 0; n < s.numberOfCores; n++)
                    {
                        threadParam w;
                    ...
                    {
                        if (distTab[n] <= threshold)
                        {
                            if (outDic.ContainsKey(allStructures[n]))
                            {
                                aux.AddRange(outDic[allStructures[n]]);
                                outDic.Remove(allStructures[n]);
                                //foreach (var item in outDic[allStructures[n]])
                                //  aux.Add(item);
                            }
                            else
                                aux.AddRange(dic[allStructures[n]]);
        ...
        private double [] GetStateInKeyStats()
        {
            double[] res = new double[2];

            if (dicC == null || dicC.Keys.Count == 0)
                return null;
        ...
        void MakeBackgroundCluster()
        {

            Dictionary<string, List<byte>> random = RandomCoverProfiles(10000, al.ProfileLength(), al.StatesStat());
            HashCluster h = new HashCluster(random, al,input);
            h.InitHashCluster();            
            List<string> names=new List<string>(random.Count);
            foreach(var item in random)
                names.Add(item.Key);
            h.PrepareClustering(names);
            ClusterOutput cl=h.Cluster(names);
        ...
        public ClusterOutput AutomaticCluster(List<string> _structNames,bool dendrog=false)
        {
            ...
            for(int i=2;i<10;i++)//relevant clusters
            {
                ...
                {
                    dicC = new Dictionary<string, List<int>>(remDic);
                    input.perData = j;
                    output.clusters = PrepareClusters(dicC, structNames);
                    aux.Add(j,EvalClustering(output.clusters));
