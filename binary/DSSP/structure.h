...
class MResidue
{
  public:
						...
						MResidue(){}
	int					SetResidue(uint32 inNumber,
							MResidue* inPrevious, const std::vector<MAtom>& inAtoms);
	...
	int					SetSSBridgeNr(uint8 inBridgeNr);
...
class MProtein
{
  public:
						...
						MProtein(char *fileName,bool inCAlphaOnly = false);
						~MProtein();
	...
	int				CalculateSecondaryStructure(bool inPreferPiHelices = true);
	int				PrepProt(std::istream& is, bool inCAlphaOnly = false);
  ...
  private:
	...
	int				AddResidue(const std::vector<MAtom>& inAtoms);
	...
	void				CalculateDihedralAngles(const std::vector<MResidue*>& inResidues);
